  ...
  return new Promise((resolve, reject) => {
    for (let school of dataset) {
      ...
      if (school.estrato_socio_economico) {
        ...
        //array = temp.split('')
        school.estrato_socio_economico = temp
      } else {
...
function geocoder (dataset) {
  return new Promise((resolve, reject) => {
    ...
    const query = {
      address: `${dataset[index].direccion}, Barranquilla, CO`
        }
    googleMapsClient.geocode(query, (err, response) => {
      if (!err) {
        if (response.json.status === 'ZERO_RESULTS'){
          ...
          dataset[index].lat = response.json.results[0].geometry.location.lat
          dataset[index].lng = response.json.results[0].geometry.location.lng
        }
        console.log(` ${index} ${dataset[index].direccion} ${dataset[index].lat}, ${dataset[index].lng}`)
        index = index + 1
...
function saveFirebaseDB (dataset) {
  ...
  for (let school of datasetFirebase) {
    let schoolRef = schools.child(school.code)
    schoolRef.set(school)
  }
}
